...
```{r, echo=FALSE, fig.alt="CONSORT flow diagram",out.width="50%",fig.align = 'center'}
knitr::include_graphics("CONSORT.png")
```

---

# Paxlovid: an oral antiviral for treating COVID-19

```{r, echo=FALSE, fig.alt="Biden has begun taking Paxlovid, an antiviral medication shown to prevent serious Covid cases",out.width="60%",fig.align = 'center'}
knitr::include_graphics("biden.png")
```

Does paxlovid reduce hospitalization or death in the first 28 days after start of treatment?
...
```{r, out.width="45%"}
vtree(stemcell, "sex prior.radiation c1c2", pattern=TRUE, showvarnames=FALSE)
```
...
```{r,out.width="88%"}
vtree(stemcell, "sex prior.radiation c1c2", sameline=TRUE, showvarnames=FALSE, summary="cmv   R %npct%", splitwidth=Inf)
```
...
##### There are lots of resources on the vtree webpage: https://nbarrowman.github.io/vtree
