...
Here's a dataset from a retrospective cohort study.
...
# Relabel variables for convenience
...
```{r, out.width="30%"}
vtree(stemcell, "cmv", summary="aKIRs", labelnode=list(cmv=c("No"=0,"Yes"=1)), labelvar=c(cmv="CMV reactivation"))
```

```{r, out.width="30%", eval=FALSE, echo=FALSE}
vtree(stemcell, "cmv", summary="aKIRs \naKIRS mean (SD) %mean% (%SD%)", labelnode=list(cmv=c("No"=0,"Yes"=1)), labelvar=c(cmv="CMV reactivation"))
```
